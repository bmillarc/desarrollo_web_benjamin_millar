...
"Ranquil","Quillon","El Carmen","Pemuco","Yungay","Chillan Viejo"]
  ...
  const poblarDepartamentos = () => {
    ...
    for (const department in region_comuna) {
        let option = document.createElement("option");
  ...
  const updateCursos = () => {
    ...
    if (region_comuna[selectedDepartment]) {
        region_comuna[selectedDepartment].forEach(course => {
            let option = document.createElement("option");
            option.value = course;
            option.text = course;
  ...
  function changeArguments() {
    const courseSelect = document.getElementById("select-comuna");
    const reasonLabel = document.querySelector("label[for='sector']");
